...
class Configuration implements ConfigurationInterface
{
    ...
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $rootNode->info('Read more about GraphQLite available options at: https://graphqlite.thecodingmachine.io/docs/symfony-bundle')
            ...
            ->arrayNode('schema')
                ->children()
                ->booleanNode('auto_reload')->defaultValue('%kernel.debug%')->info('Check controller and type files for changes on every request, so that an edit shows up without clearing the cache. Defaults to kernel.debug.')->end()
                ->end()
            ->end()
            ->arrayNode('security')
                ->children()
